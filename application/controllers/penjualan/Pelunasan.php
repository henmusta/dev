...
class Pelunasan extends GT_Controller {
	private $module = [
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'penjualan/Pelunasan_model' );
	...
	public function update($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'pelunasan'	=> $this->Pelunasan_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'delete') ) ){
			...
			$response 	= $this->Pelunasan_model->{str_replace('-','_',$action)}($params);
	...
	public function single($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'penjualan'		=> $this->Pelunasan_model->single($pk),
	...
	public function api_data($vendor_name = null){
		...
		$this->load->model('master/Pelunasan_model');
		if( method_exists($this->Pelunasan_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Pelunasan_model->{$vendor_name}($config);
