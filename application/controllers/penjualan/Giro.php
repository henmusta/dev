...
class Giro extends GT_Controller {
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'penjualan/Giro_jual_model' );
	}
	...
	{	
		$this->db->set('chek','1');
		$this->db->where('id',$id);
		$this->db->update('rincian_pelunasan');
	}
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Giro_jual_model, $vendor_name) ){
			...
			endswitch;
			$response 	= $this->Giro_jual_model->{$vendor_name}($config);
		}
		$this->output
