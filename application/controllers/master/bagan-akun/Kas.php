...
class Kas extends GT_Controller {
	private $module = [
		'name' 	=> 'Kas',
		'url'	=> 'master/bagan-akun/kas',
	];
	public function __construct(){
		parent::__construct();
		$this->load->model( 'master/bagan-akun/Kas_model' );
	}
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'update-status', 'delete') ) ){
			...
			$response 	= $this->Kas_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Kas_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Kas_model->{$vendor_name}($config);
