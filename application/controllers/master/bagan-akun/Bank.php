...
class Bank extends GT_Controller {
	private $module = [
		'name' 	=> 'Bank',
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'master/bagan-akun/Bank_model' );
	}
	...
	public function update($pk = null){
		...
		$data = array(
			'module' 	=> $this->module,
			'data'		=> $this->Bank_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/sweetalert2/sweetalert2.min.css',
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'update-status', 'delete') ) ){
			...
			$response 	= $this->Bank_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Bank_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Bank_model->{$vendor_name}($config);
