...
class pelanggan extends GT_Controller {
	private $module = [
		'name' 	=> 'Pelanggan',
	...
	public function update($pk = 0){
		...
		$data = array(
			'module' 	=> $this->module,
			'data'		=> $this->Pelanggan_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'update-status', 'delete') ) ){
			...
			$response 	= $this->Pelanggan_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		$this->load->model('master/Pelanggan_model');
		if( method_exists($this->Pelanggan_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Pelanggan_model->{$vendor_name}($config);
