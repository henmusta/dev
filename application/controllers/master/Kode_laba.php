...
class Kode_laba extends GT_Controller {
	private $module = [
		'name' 	=> 'Kode Laba',
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'master/Kode_laba_model' );
	}
	...
	public function update($pk = 0){
		...
		$data = array(
			'module' 	=> $this->module,
			'data'		=> $this->Kode_laba_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'update-status', 'delete') ) ){
			...
			$response 	= $this->Kode_laba_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		$this->load->model('master/Kode_laba_model');
		if( method_exists($this->Kode_laba_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			switch($vendor_name):
				case 'datatable':
					$config = array(
				...
				break;
			endswitch;
			$response 	= $this->Kode_laba_model->{$vendor_name}($config);
