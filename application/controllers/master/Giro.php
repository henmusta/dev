...
class giro extends GT_Controller {
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'master/Giro_model' );
	}
	...
	public function update($pk = null){
		...
		$data = array(
			'module' 	=> $this->module,
			'data'		=> $this->Giro_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/sweetalert2/sweetalert2.min.css',
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'delete', 'deleteall') ) ){
			$params 	= $this->input->post(NULL, TRUE);
			$response 	= $this->Giro_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Giro_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Giro_model->{$vendor_name}($config);
