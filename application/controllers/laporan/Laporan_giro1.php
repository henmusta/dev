...
class Laporan_giro1 extends GT_Controller {
	private $module = [
        'name' 	=> 'Laporan Giro',
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'laporan/Lap_giro1' );
	}
	public function index(){
		...
		$foot = array(
			...
			'scripts' 		=> array(
				$this->load->view($this->module['url'] . '/javascript/datatable1', $data, TRUE)
			)
		);

		$this->html_head($head);
		$this->load->view($this->module['url'] . '/html/datatable1',$data);
		$this->html_foot($foot);
	}
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Lap_giro1, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			switch($vendor_name):
				case 'datatable':
					$config = array(
			...
			$response 	= $this->Lap_giro1->{$vendor_name}($config);
