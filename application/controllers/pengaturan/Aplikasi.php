...
class Aplikasi extends GT_Controller {
	private $module = [
		'name' 	=> 'aplikasi',
	...
	public function index(){
		...
		$foot = array(
			'javascripts'	=> array(
				...
				'assets/js/plugins/datatables/RowGroup-1.1.2/js/rowGroup.bootstrap4.min.js',
			...
			'scripts' 		=> array(
				$this->load->view($this->module['url'] . '/js/datatables', $data, TRUE)
			)
		);

		$this->html_head($head);
		$this->load->view($this->module['url'] . '/html/datatables',$data);
		$this->html_foot($foot);
	}
	...
	public function update($pk = null){
		...
		$data = array(
			'module' 	=> $this->module,
			'data'		=> $this->Aplikasi_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/sweetalert2/sweetalert2.min.css',
		...
		$foot = array(
			...
			'scripts' 		=> array(
				$this->load->view($this->module['url'] . '/js/form', $data, TRUE)
			)
		);
		$this->html_head($head);
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('update') ) ){

		
			$params 	= $this->input->post(NULL, TRUE);
			$response 	= $this->Aplikasi_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Aplikasi_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Aplikasi_model->{$vendor_name}($config);
