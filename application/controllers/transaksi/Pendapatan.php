...
class Pendapatan extends GT_Controller {
	private $module = [
		'name' 	=> 'pendapatan',
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'transaksi/Pendapatan_model' );
	}
	...
	public function update($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'data'			=> $this->Pendapatan_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'delete') ) ){
			...
			$response 	= $this->Pendapatan_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		$this->load->model('master/Pendapatan_model');
		if( method_exists($this->Pendapatan_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Pendapatan_model->{$vendor_name}($config);
