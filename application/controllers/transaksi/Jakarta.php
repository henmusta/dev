...
class jakarta extends GT_Controller {
	private $module = [
		'name' 	=> 'Transaksi_jakarta',
		'url'	=> 'transaksi/jakarta',
	];
	public function __construct(){
		parent::__construct();
		$this->load->model( 'transaksi/Jakarta_model' );
	}
	...
	public function insert(){
		...
		$data = array(
			'module' => $this->module,
			'data'	 => $this->Jakarta_model->singleadd() 
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function update($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'data'			=> $this->Jakarta_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'delete') ) ){
			...
			$response 	= $this->Jakarta_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Jakarta_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Jakarta_model->{$vendor_name}($config);
