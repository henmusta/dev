...
class Kasir extends GT_Controller {
	private $module = [
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'transaksi/Kasir_model' );
	}
	...
	public function insert($pk = null){
		$cb = $this->uri->segment(5);
		$this->module['action'] = $this->module['url'] . '/crud/insert';
		$data = array(
			'tgl' 		=> $pk,
			'module' => $this->module,
			'data'	 => $this->Kasir_model->single_pendapatan($pk, $cb)
		);
		...
		$foot = array(
			...
			'scripts' 		=> array(
				$this->load->view($this->module['url'] . '/javascript/dump1', $data, TRUE)
			)
		);
		$this->html_head($head);
		$this->load->view($this->module['url'] . '/html/dump', $data);
		$this->html_foot($foot);
	}
	public function update(){
		...
		$data = array(
			'tgl' 		=> $tgl,
			'module' 		=> $this->module,
			'data'			=> $this->Kasir_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'delete') ) ){
			...
			$response 	= $this->Kasir_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Kasir_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			switch($vendor_name):
				case 'datatable':
					$config = array(
				...
				break;
			endswitch;
			$response 	= $this->Kasir_model->{$vendor_name}($config);
		}
		$this->output
			->set_status_header(200)
			->set_content_type('Application/json')
	...
	public function single($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'transaksi'		=> $this->Kasir_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(),
