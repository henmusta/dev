...
class Transfer extends GT_Controller {
	private $module = [
		'name' 	=> 'Transfer',
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'transaksi/Transfer_model' );
	}
	...
	public function update($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'data'			=> $this->Transfer_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'delete') ) ){
			...
			$response 	= $this->Transfer_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		$this->load->model('master/Transfer_model');
		if( method_exists($this->Transfer_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Transfer_model->{$vendor_name}($config);
