...
class Stok_opname extends GT_Controller {
	private $module = [
		'name' 	=> 'Stok opname',
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'inventori/Stok_opname_model' );
	}
	...
	public function update($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'data'			=> $this->Stok_opname_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function single($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'stok_opname'		=> $this->Stok_opname_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(),
			'heading' => array('title'=>$this->module['name'])
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert','update-harga', 'update', 'delete') ) ){
			...
			$response 	= $this->Stok_opname_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Stok_opname_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Stok_opname_model->{$vendor_name}($config);
