...
class Faktur extends GT_Controller {
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'pembelian/Faktur_model' );
	...
	public function harga_jual($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'pembelian'		=> $this->Faktur_model->single($pk, $satuan)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Faktur_model, $vendor_name) ){
			...
			switch($vendor_name):
				...
				break;
				case 'autocomplete_pemasok' :
				case 'autocomplete_produk' :
					$config = $this->input->get(NULL, TRUE);
					break;
			endswitch;
