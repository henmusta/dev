...
class Pembayaran extends GT_Controller {
	private $module = [
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'pembelian/Pembayaran_model' );
	...
	public function single($pk = null){
		...
		$data = array(
			...
			'pembayaran'	=> $this->Pembayaran_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(),
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'delete') ) ){
			...
			$response 	= $this->Pembayaran_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		$this->load->model('master/Pembayaran_model');
		if( method_exists($this->Pembayaran_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Pembayaran_model->{$vendor_name}($config);
