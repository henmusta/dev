...
class Retur extends GT_Controller {
	private $module = [
	...
	public function __construct(){
		parent::__construct();
		$this->load->model( 'pembelian/Retur_model' );
	}
	...
	public function update($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'data'			=> $this->Retur_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function harga_jual($pk = null){
		...
		$data = array(
			'module' 		=> $this->module,
			'pembelian'		=> $this->Retur_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(
				'assets/js/plugins/datatables/DataTables-1.10.21/css/dataTables.bootstrap4.min.css',
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert','update-harga', 'update', 'delete') ) ){
			...
			$response 	= $this->Retur_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		if( method_exists($this->Retur_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Retur_model->{$vendor_name}($config);
