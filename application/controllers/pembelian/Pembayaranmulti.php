...
class Pembayaranmulti extends GT_Controller {
	private $module = [
		'name' 	=> 'Pembayaran Pembelian Multi',
		'url'	=> 'pembelian/pembayaranmulti',
	];
	public function __construct(){
		parent::__construct();
		$this->load->model('pembelian/Pembayaranmulti_model');
	...
	public function single($pk = null){
		...
		$data = array(
			...
			'pembayaran'	=> $this->Pembayaranmulti_model->single($pk)
		);
		$head = array(
			'stylesheets'		=> array(),
	...
	public function crud($action = null){
		...
		if( !empty($action) && in_array($action, array('insert', 'update', 'delete') ) ){
			...
			$response 	= $this->Pembayaranmulti_model->{str_replace('-','_',$action)}($params);
	...
	public function api_data($vendor_name = null){
		...
		$this->load->model('pembelian/Pembayaranmulti_model');
		if( method_exists($this->Pembayaranmulti_model, $vendor_name) ){
			$params = $this->input->post(NULL, TRUE);
			...
			$response 	= $this->Pembayaranmulti_model->{$vendor_name}($config);
