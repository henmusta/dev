...
class Pelanggan_model extends CI_Model {
	...
	private function is_unique_field($column_name, $value, $pk=NULL){
		$query = "SELECT COUNT(`id`) AS `total` FROM `pelanggan` WHERE `". $column_name ."`='". $this->db->escape_str($value) ."' ";
		if(!empty($pk)){
			$query .= " AND `id`!='" . $pk ."'";
		}
	...
	public function single($pk){
		return $this->db->get_where('pelanggan',array('id'=>$pk))->row();
	}
	public function datatable($config = array()){
		
		extract($config);
	$columns 			= array('pelanggan.kode', 'pelanggan.nama', 'telp', 'pelanggan.email', 'pelanggan.alamat', 'COUNT(penjualan.id)', 'belanja');

		$select_total 		= "SELECT COUNT(pelanggan.id) AS `total` ";
		$select_filtered	= "SELECT FOUND_ROWS() AS `total` ";
		$select 			= "SELECT SQL_CALC_FOUND_ROWS pelanggan.*, penjualan.*, sum(penjualan.total_pelunasan) AS belanja, pelanggan.id as idp, COUNT(penjualan.id) as total_belanja ";
		$from 				= "FROM `pelanggan` 
							   left join penjualan 
	...
	public function insert($params){
		...
		if( isset($pelanggan['kode']) && !empty($pelanggan['kode']) && isset($pelanggan['nama']) && !empty($pelanggan['nama']) ){
			
			if( isset($pelanggan['kode']) && !empty($pelanggan['kode']) ){
				$pelanggan['kode'] 	= trim(strip_tags($pelanggan['kode']));
				$data_is_valid 	= $this->is_unique_field('kode', $pelanggan['kode']);
				if($data_is_valid == FALSE){
					$result['message'] 	= "Kode sudah digunakan.";
				} else {
					if( isset($pelanggan['nama']) && !empty($pelanggan['nama']) ){
						$pelanggan['nama'] 	= trim(strip_tags($pelanggan['nama']));
						$data_is_valid 	= $this->is_unique_field('nama', $pelanggan['nama']);
						if($data_is_valid == FALSE){
			...
			if( $data_is_valid == TRUE ){
				...
				if( $this->db->insert('pelanggan', $pelanggan) ){
	...
	public function update($params){
		...
		if( isset($pelanggan['kode']) && !empty($pelanggan['kode']) && isset($pelanggan['nama']) && !empty($pelanggan['nama']) ){
			if( isset($pelanggan['kode']) && !empty($pelanggan['kode']) ){
				$pelanggan['kode'] 	= trim(strip_tags($pelanggan['kode']));
				$data_is_valid 	= $this->is_unique_field('kode', $pelanggan['kode'], $pk);
				if($data_is_valid == FALSE){
					$result['message'] 	= "Kode sudah digunakan.";
				} else {
					if( isset($pelanggan['nama']) && !empty($pelanggan['nama']) ){
						$pelanggan['nama'] 	= trim(strip_tags($pelanggan['nama']));
						$data_is_valid 	= $this->is_unique_field('nama', $pelanggan['nama'], $pk);
						if($data_is_valid == FALSE){
							$result['message'] 	= "Nama sudah digunakan.";
						}
					}
			...
			if( $data_is_valid == TRUE ){
				if( is_array($pelanggan) && count($pelanggan) > 0 ){
					$result['message'] 	= "Data dagal disimpan.";
					if( $this->db->update('pelanggan', $pelanggan, array('id'=>$pk)) ){
						$result['status'] 	= TRUE;
						$result['message'] 	= 'Data telah disimpan.';
					}
				}
