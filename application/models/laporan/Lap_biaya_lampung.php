...
class Lap_biaya_lampung extends CI_Model {
	...
	public function datatable($config = array()){
		...
		LEFT JOIN transaksi 
		ON rincian_biaya_lampung.`id_transaksi` = transaksi.`id`     
		";
		$where 				= "WHERE transaksi.`tipe` = 'Transaksi_lampung' ";
		$group_by 			= "Order by tgl DESC ";
		$having 			= "";
		$order_by 			= "";
		$limit 				= "";
		...
		if( isset($params["filter"]["start_date"]) && isset($params["filter"]["end_date"]) ){
			...
			$where .= " AND (DATE(rincian_biaya_lampung.`tgl`) BETWEEN DATE('". $start ."') AND DATE('". $end ."'))";
		}
    ...
    {   
		...
		->where('rincian_biaya_lampung.tgl BETWEEN "'. date('Y-m-d', strtotime($start)). '" and "'. date('Y-m-d', strtotime($end)).'"')
