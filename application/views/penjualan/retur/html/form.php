...
<div class="content">
	<form id="form" method="POST" action="<?= isset($module['action']) ? $module['action'] : NULL ;?>" autocomplete="off">
		...
		<input type="hidden" name="pk_faktur" value="<?= isset($retur->retur_jual->id_jual) ? $retur->retur_jual->id_jual : NULL;?>">
				...
				<div class="row">
					<div class="col-md-6">
						...
						<div class="form-group form-row">
							...
							<div class="col-md-9">
								<select id="select-pelanggan" class="form-control" name="retur[id_pelanggan]">
								<?= isset($retur->pelanggan->id) ? '<option value="'. $retur->pelanggan->id .'" selected="selected">'. $retur->pelanggan->nama .'</option>' : NULL ;?>
								</select>
							</div>
						</div>
						<div class="form-group form-row">
							...
							<div class="col-md-9">
								<select id="select2-penjualan" class="form-control select2-penjualan" name="retur[id_penjualan]">
								<?= isset($retur->retur_jual->id_jual) ? '<option value="'. $retur->retur_jual->id_jual .'" selected="selected">'. $retur->retur_jual->nomor .'</option>' : NULL ;?>
								</select>
							</div>
						</div>
					</div>
