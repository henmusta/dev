...
$(function(){
	...
	$(document).ready(function(){
			...
			$.post(
				...
				{nama:nama}, function(response){
					if(response.status == 'success'){
						$('#kode-pelanggan').val(response.kode_pelanggan);
					} else {
						$('#kode-pelanggan').val(null);
					}
				},
				'json'
			);
		...
		const billItems 			= new AutoNumeric('#billItems',currenciesOptions);
				...
				{ 
					...
					render 		: function ( columnData, type, rowData, meta ) {
						return String(`
							...
							<input type="hidden" id="qty_harga` + meta.row + `" class="form-control text-right" value="`+ rowData.harga_jual +`" name="rincian[`+ meta.row +`][qty_harga]" data-column="qty_komputer">
						`).trim();
					}
				},
				{ 
			...
			createdRow : function( row, data, index ){
				...
				$(row).find('.select2-produk').select2({
					ajax : {
						...
						processResults: function (myData) {
							var data = $.map(myData.results, function (obj) {
								obj.text = obj.text || obj.kode +' / '+ obj.nama_pemasok +' / '+ obj.nama_produk;
								return obj;
							});
							return {
								results : data
					...
					'select2:select' : function(e){
						...
						AutoNumeric.getAutoNumericElement('#qty_harga' + index).set(data.harga_jual);
