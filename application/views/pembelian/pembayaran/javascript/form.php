...
$(function(){
	...
	$(document).ready(function(){
		...
		$('#select2-pembelian').select2({
			...
			change : function(){		
				// date('Y-m-d');
				$.post("<?= $module['url'];?>/api-data/get-pembelian",{pk:$(this).val()},function(response){
					$('#total_tagihan').html($.fn.dataTable.render.number( ',', '.', 0, '' ).display(response.total_tagihan));
					$('#total_pembayaran').html($.fn.dataTable.render.number( ',', '.', 0, '' ).display(response.total_pembayaran));
					$('#sisa_tagihan').html($.fn.dataTable.render.number( ',', '.', 0, '' ).display(response.sisa_tagihan));
					$('#tglbayar').val(new Date().toISOString().slice(0, 10));
					$('#tglnota').val(response.tgl_nota);
					$('#tglbuat').val(response.tgl_buat);
					$('#nomor').val(response.nomor);
					$('#bill').val(response.sisa_tagihan);
				},'json');	
			}
		});
	});
