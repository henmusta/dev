	...
	<div class="row">
		<div class="col-md-8">
			<form id="form" method="POST" action="<?= isset($module['action']) ? $module['action'] : NULL ;?>" autocomplete="off">
				...
				<input type="hidden" id="id_cabang" name="pelanggan[id_cabang]" value="<?php echo $this->user->id_cabang;?>">
				<div class="block">
					...
					<div class="block-content">						
						<div class="form-group form-row align-items-center">
							...
							<div class="col-md-4">
								<input type="text" class="form-control" name="pelanggan[kode]" required="required" value="<?= isset($data->kode) ? $data->kode : NULL ;?>">
							</div>
						</div>
						<div class="form-group form-row align-items-center">
							<label class="col-md-2 text-right">Nama</label>
							<div class="col-md-8">
								<input type="text" class="form-control" name="pelanggan[nama]" required="required" value="<?= isset($data->nama) ? $data->nama : NULL ;?>">
							</div>
						</div>
						<div class="form-group form-row align-items-center">
							<label class="col-md-2 text-right">Telp</label>
							<div class="col-md-4">
								<input type="text" class="form-control" name="pelanggan[telp]" value="<?= isset($data->telp) ? $data->telp : NULL ;?>">
							</div>
						</div>
						<div class="form-group form-row align-items-center">
							<label class="col-md-2 text-right">Email</label>
							<div class="col-md-4">
								<input type="email" class="form-control" name="pelanggan[email]" value="<?= isset($data->email) ? $data->email : NULL ;?>">
							</div>
						</div>
						<div class="form-group form-row align-items-center">
							<label class="col-md-2 text-right">Alamat</label>
							<div class="col-md-8">
								<textarea class="form-control" name="pelanggan[alamat]"><?= isset($data->alamat) ? $data->alamat : NULL ;?></textarea>
							</div>
						</div>						
					</div>
				</div>
