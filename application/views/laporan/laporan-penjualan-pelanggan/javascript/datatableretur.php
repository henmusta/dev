	...
	$(document).ready(function(){
		...
		const dt = $('#dt').DataTable({
           ...
           var pageTotal = api
			...
			serverSide 		: true,
			ajax:{
				url 		: "<?= $module['url'];?>/api-data-retur/datatableretur", 
				type 		: "POST",
				data 		: function(d){
					return $.extend({},d,{
						filter : {
			...
			columns 		: [
				...
				// { data : 'nomor', width: '20px',  className : 'text-center'},
				{ data : 'qty_retur', className : 'text-center' },
				{ data : 'namap' },
				{ data : 'harga', className:'text-right', width:'180px', render: $.fn.dataTable.render.number( ',', '.', 0, 'Rp.' ) },
				{ data : 'total', className:'text-right', width:'180px', render: $.fn.dataTable.render.number( ',', '.', 0, 'Rp.' ) }
				// { 
				// 	data : 'id', 
				...
				// 		return `
				// 		<a class="dropdown-item" href="<?= $module['url'];?>/single/`+ columnData +`">Detail</a>
				// 		`;
				// 	}
				// }
			]
		...
		$( "#excel" ).click(function() {
         ...
         window.open("<?php echo site_url('laporan/laporan_retur_penjualan/excel/')?>"+date_start+'/'+date_end+'/'+id_cabang)
